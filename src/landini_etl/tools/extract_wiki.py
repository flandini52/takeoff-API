...
export. Ad hoc exploration only — the deadlines/ pipeline has its own,
narrower flattening logic tailored to the dashboard.

Ported from the old root-level extract_wiki.py: same behavior, only the
import of TakeoffClient changed (now local to the ETL package).

Usage (inside the etl container):
    python -m app.tools.extract_wiki "Landini Srl" [--exact] [--format csv|jsonl] [--output path]
"""
