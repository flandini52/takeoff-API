"""Generic exploration CLI: export any paginated Takeoff CRM entity list to
JSONL/CSV on disk. Not part of the activities/deadlines pipelines — for ad
hoc exploration of entities that don't have a dedicated extract/ module
yet (contacts, jobs, estimates, tasks, contracts, invoices, ...).

Ported from the old root-level extract_data.py: same behavior, only the
import of TakeoffClient changed (now local to the ETL package).

Usage (inside the etl container):
    python -m app.tools.extract_data <entity> [--take 100] [--max-records N]
        [--format jsonl|csv] [--output path] [--filter '{"key": "value"}']
"""
