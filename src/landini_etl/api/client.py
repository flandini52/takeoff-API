"""Client for the Takeoff CRM API (TakeOff Integrators API).

https://webapi.takeoffcrm.com — swagger:
https://webapi.takeoffcrm.com/swagger/v1/swagger.json

Ported from the old root-level takeoff_client.py: same behavior, only the
config import changed (now local to the ETL package). This is the single
point of contact with the gestionale — extract/ modules and tools/ all go
through this class, never call `requests` directly.
"""
